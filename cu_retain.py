...
EMPTY_STOP_PATIENCE   = 2 # stop after this many consecutive empty-triplet epochs #changed from 2

# Number of WikiText-103 passages to sample for the retain set.
# Drawn from the test split with a fixed seed for reproducibility.
RETAIN_SIZE = 50
RETAIN_MIN_CHARS = 200  # minimum character length for a passage to qualify
RETAIN_RANDOM_SEED = 42
...
# =============================================================================
# GET_RETAIN — fixed WikiText-103 passages filtered by forbidden entities
# =============================================================================

def get_retain(forbidden_entities: list[str]) -> list[str]:
    """
    Loads a fixed, reproducible slice of the WikiText-103 test split to serve
    as the retain set for L3.  Passages that mention any forbidden entity are
    excluded so the retain signal cannot inadvertently reinforce knowledge the
    unlearning procedure is trying to erase.

    Design rationale
    ----------------
    * WikiText-103 is drawn from the same Wikipedia distribution that Llama
      was pretrained on, making it the closest publicly available approximation
      to the true pretraining distribution.
    * Using the *test* split avoids any overlap with WikiText-103 training
      data that may have appeared in pretraining corpora.
    * A fixed random seed (RETAIN_RANDOM_SEED) and fixed sample size
      (RETAIN_SIZE) guarantee that the retain set is identical across runs,
      ablations, and entity choices — a necessary condition for fair comparison.
    * Filtering is case-insensitive exact substring match.  This is
      conservative: it may exclude passages that only mention a forbidden
      entity incidentally, but guarantees that no forbidden-entity knowledge
      leaks into the retain gradient.  Callers should pass all entities being
      unlearned in the full experimental suite (not just the current run) so
      that the same retain corpus can be reused across all conditions.

    :param forbidden_entities: list of entity strings to exclude.  Should
        include all forget targets across the full experiment (in all
        languages / transliterations) so a single retain set can be shared.
        Matching is case-insensitive substring search.
    :return: list of passage strings (Xretain) for use in compute_l3_loss
    :rtype: list[str]
    """
    from datasets import load_dataset
    import random

    print("\n[GET_RETAIN] Loading WikiText-103 test split ...")
    dataset = load_dataset("wikitext", "wikitext-103-v1", split="test")

    # Normalise forbidden entities once for efficient repeated comparison
    forbidden_lower = [e.lower() for e in forbidden_entities if e]

    def is_clean(text: str) -> bool:
        """Return True if the passage contains no forbidden entity mention."""
        t = text.lower()
        return not any(f in t for f in forbidden_lower)

    # Filter: minimum length and no forbidden entity mentions
    candidates = [
        row["text"].strip()
        for row in dataset
        if len(row["text"].strip()) >= RETAIN_MIN_CHARS and is_clean(row["text"])
    ]

    if len(candidates) < RETAIN_SIZE:
        raise ValueError(
            f"[GET_RETAIN] Only {len(candidates)} passages survived filtering "
            f"(need {RETAIN_SIZE}).  Consider reducing RETAIN_SIZE or "
            f"RETAIN_MIN_CHARS, or narrowing the forbidden_entities list."
        )

    rng = random.Random(RETAIN_RANDOM_SEED)
    selected = rng.sample(candidates, RETAIN_SIZE)

    print(
        f"[GET_RETAIN] {len(candidates)} candidates after filtering "
        f"({len(dataset) - len(candidates)} excluded for forbidden entities or length), "
        f"sampled {len(selected)} with seed={RETAIN_RANDOM_SEED}."
    )
    return selected
...
# =============================================================================
# L3: Retain loss — normal cross-entropy on WikiText-103 passages
# =============================================================================

def compute_l3_loss(model, tokenizer, Xretain: list[str]) -> torch.Tensor:
    """
    Normal (positive) cross-entropy on passages from the WikiText-103 retain
    set.  Gradient descent here pulls the trainable model back toward its
    original language-modelling behaviour on text unrelated to the forget
    target, counteracting the distributional drift caused by L1/L2 gradient
    ascent.
    """
    device = next(model.parameters()).device
    total_loss = torch.tensor(0.0, device=device)
    ...
    for text in Xretain:
        ...
        outputs = model(input_ids=ids, labels=ids)
        total_loss = total_loss + outputs.loss  # positive loss: normal gradient descent
        count += 1
...
def run_unlearning(forget_entity: str, languages: list[str], prompt_mode: str = "native", max_epochs: int = MAX_EPOCHS, output_dir: str = "./unlearned_model_output", max_epochs_spanish: int = None, forbidden_entities: list[str] = None): # CHANGED: added forbidden_entities parameter
    ...
    #added to speed up training
    try:
        model = torch.compile(model)
        print("torch.compile applied to trainable model.")
    ...
    # ── GET_RETAIN: load fixed WikiText-103 passages, excluding all forbidden entities ──
    # all_forbidden pools the current entity's translated names with any caller-supplied
    # list so that a shared retain corpus can be used cleanly across ablations.
    all_forbidden = list(entity_by_language.values())  # translated names of current entity
    if forbidden_entities:
        all_forbidden.extend(forbidden_entities)  # additional entities caller wants excluded
    Xretain = get_retain(forbidden_entities=all_forbidden)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # L3 update: retain loss — gradient descent on WikiText-103 to prevent collapse
        optimizer.zero_grad()
        l3_loss = compute_l3_loss(model, tokenizer, Xretain)
        l3_loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        optimizer.step()
        ...
        print(
            ...
            f"L3={l3_loss.item():.4f} | "
            f"triplet pairs={len(Xent_all)}"
        )

    # Algorithm 1, Line 7: save unlearned model
    login(HF_TOKEN)  # or set HF_TOKEN env var

    repo_id = "Ruth-Ann/multi_mc"
...
#    output_dir = f"./unlearned_model_output_{langs_tag}_{args.prompt_mode}_{timestamp}{tag_part}"
    ...
    try:
        run_unlearning(args.entity, args.languages, args.prompt_mode, args.max_epochs, output_dir, args.max_epochs_spanish, args.forbidden_entities) # CHANGED: pass forbidden_entities
    except Exception as e:
